...
GPU 0과 2만 PyTorch에 "보이며" 각각 `cuda:0`과 `cuda:1`에 매핑됩니다.  
...
XPU 0과 2만 PyTorch에 "보이며" 각각 `xpu:0`과 `xpu:1`에 매핑됩니다.  
...
> [!WARNING]
> 환경 변수는 명령줄에 추가하는 대신 export될 수 있습니다. 환경 변수가 어떻게 설정되었는지 잊어버려서 잘못된 가속기를 사용하게 될 수 있어 혼란스러울 수 있으므로 이는 권장되지 않습니다. 대신, 동일한 명령줄에서 특정 훈련 실행에 대해 환경 변수를 설정하는 것이 일반적인 관례입니다.
```